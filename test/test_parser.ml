...
let test_paren_power () =
  (* (a-b)^2 = a^2 - 2ab + b^2 *)
  parses_to "(a-b)^2" (Polynomial.pow a 2 -! scale 2 1 (a *! b) +! Polynomial.pow b 2) ()
;;
...
let test_claim_ge () =
  ...
  | Ok claim ->
    ...
    Alcotest.check poly "target = A - B" (Prover.hello_world_target ()) target
;;
...
let test_clear_square_over_const () =
  ...
  Alcotest.check
    poly
    "(a-b)^2/2 clears"
    (scale 2 1 (Polynomial.pow (a -! b) 2))
...
let test_json_wrong_coeff () =
  ...
  | Ok { target; certificate; _ } ->
    Alcotest.(check bool)
      "wrong coefficient is rejected by checker"
      false
      (Checker.check_sos target certificate)
;;
  ...
  Alcotest.run
    ...
    [ ( "expressions"
      , [ Alcotest.test_case "sum of powers" `Quick test_sum
        ...
        ; Alcotest.test_case
            "right-assoc power rejected"
            `Quick
            test_right_assoc_power_rejected
        ] )
    ; ( "claims"
    ...
    ; ( "json"
      , [ Alcotest.test_case "valid certificate loads and checks" `Quick test_json_valid
        ...
        ; Alcotest.test_case
            "malformed rational rejected"
            `Quick
            test_json_malformed_rational
        ...
        ; Alcotest.test_case
            "empty SOS proves identity"
            `Quick
            test_json_empty_sos_identity
