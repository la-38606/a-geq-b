...
(* Degree-<=2 targets the Stage B prover should handle. *)
let provable_cases =
  [ "a^2 + b^2 >= 2*a*b";
...
(* Must NOT be proved: false claims, out-of-scope (needs constraints), and
   higher-degree targets Stage B does not yet handle. *)
let unprovable_cases =
  [ "a^2 + b^2 >= 3*a*b" (* false *);
    ...
    "a^4 + b^4 >= 2*a^2*b^2" (* degree 4: beyond Stage B *);
    "a^3 + b^3 + c^3 >= 3*a*b*c" (* degree 3: beyond Stage B *) ]
