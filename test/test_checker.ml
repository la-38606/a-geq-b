...
let test_accepts_valid () =
  Alcotest.(check bool) "valid SOS certificate accepted" true
    (Checker.check_sos target valid_cert)

let test_expand_equals_target () =
  Alcotest.(check bool) "expansion equals the target polynomial" true
...
let test_rejects_negative_coeff () =
  ...
  in
  Alcotest.(check bool) "negative coefficient rejected" false
  ...
  let bad =
    Certificate.make
      [ Certificate.term half (Polynomial.sub a two_b);
        ...
        Certificate.term half (Polynomial.sub a c) ]
  in
  Alcotest.(check bool) "wrong expansion rejected" false
    (Checker.check_sos target bad)
  ...
  let bad =
    Certificate.make
      [ Certificate.term half (Polynomial.sub a b);
        Certificate.term half (Polynomial.sub b c) ]
  in
  Alcotest.(check bool) "missing term rejected" false
    (Checker.check_sos target bad)
...
let test_rejects_extra_term () =
  ...
  in
  Alcotest.(check bool) "extra term rejected" false
    (Checker.check_sos target bad)
    ...
    [ ( "accept",
      ...
      ( "reject",
        [ Alcotest.test_case "negative coefficient" `Quick test_rejects_negative_coeff;
          ...
          Alcotest.test_case "reason is negative-coefficient" `Quick test_failure_reason_negative ] ) ]
