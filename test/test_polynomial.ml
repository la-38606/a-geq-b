...
let for_all_pairs f =
  List.iter (fun p -> List.iter (fun q -> f p q) samples) samples
...
let test_zero_terms_removed () =
  Alcotest.(check bool) "a - a is the empty (zero) polynomial" true
...
let test_degree_and_vars () =
  Alcotest.(check int) "deg((a-b)^2) = 2" 2 (Polynomial.degree (Polynomial.pow (a -! b) 2));
...
let test_mul_commutes () =
  for_all_pairs (fun p q -> check "p*q = q*p" (p *! q) (q *! p))

let test_add_assoc () =
  for_all_triples (fun p q r ->
      check "(p+q)+r = p+(q+r)" ((p +! q) +! r) (p +! (q +! r)))
  ...
  Alcotest.run "polynomial"
    [ ( "operations",
        [ Alcotest.test_case "add combines like terms" `Quick test_add_combines_like_terms;
          ...
          Alcotest.test_case "equality ignores construction" `Quick test_equal_ignores_construction;
