...
let of_json (json : Yojson.Safe.t) : (parsed, string) result =
  ...
  let poly_of_string ~vars src =
    ...
    | Ok e -> (
        try Normalizer.poly_of_expr vars e
        with Invalid_argument m -> raise (Bad m))
  in
  try
    let claim_text =
      try json |> U.member "claim" |> U.to_string
      with _ -> raise (Bad "missing or non-string field 'claim'")
    in
    let vars =
      try json |> U.member "variables" |> U.to_list |> List.map U.to_string
      with _ -> raise (Bad "missing or invalid field 'variables'")
    in
    let sos =
      try json |> U.member "sos" |> U.to_list
      with _ -> raise (Bad "missing or invalid field 'sos'")
    in
    let target =
      match Parser.parse claim_text with
      | Error m -> raise (Bad ("cannot parse claim: " ^ m))
      | Ok c -> (
          try snd (Normalizer.poly_of_claim ~context:vars c)
          with Invalid_argument m -> raise (Bad m))
    in
    let parse_term j =
      let coeff_s =
        try j |> U.member "coeff" |> U.to_string
        with _ -> raise (Bad "certificate term is missing a string 'coeff'")
      in
      let poly_s =
        try j |> U.member "poly" |> U.to_string
        with _ -> raise (Bad "certificate term is missing a string 'poly'")
      in
      let coeff =
