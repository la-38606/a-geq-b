...
let vars_of_expr (e : Ast.expr) : context =
  ...
  let rec go = function
    ...
    | Ast.Neg a -> go a
    | Ast.Pow (a, _) -> go a
    | Ast.Add (a, b) | Ast.Sub (a, b) | Ast.Mul (a, b) | Ast.Div (a, b) -> go a; go b
  in
  go e;
  List.rev !seen
...
let rf_div (na, da) (nb, db) =
  if Polynomial.is_zero nb then invalid_arg "Normalizer: division by zero"
...
let poly_of_expr (ctx : context) (e : Ast.expr) : Polynomial.t =
  ...
  if Polynomial.degree d = 0 then
    ...
    invalid_arg
      "Normalizer.poly_of_expr: expression is not a polynomial (division by a non-constant)"
...
let poly_of_claim ?(context : context option) (c : Ast.claim) : context * Polynomial.t =
  ...
  let cleared = Polynomial.mul num den in (* (A - B) * den^2 *)
  let p = match c.Ast.op with Ast.Ge -> cleared | Ast.Le -> Polynomial.neg cleared in
  (ctx, p)
