...
let tokenize (s : string) : token list =
  ...
  while !i < n do
    ...
    | ')' -> push RPAREN; incr i
    | '>' ->
        if !i + 1 < n && s.[!i + 1] = '=' then (push GE; i := !i + 2)
        else raise (Error_msg "expected '>=' after '>'")
    | '<' ->
        if !i + 1 < n && s.[!i + 1] = '=' then (push LE; i := !i + 2)
        else raise (Error_msg "expected '<=' after '<'")
    | _ when is_digit c ->
        let j = ref !i in
...
let rec p_base (c : cursor) : Ast.expr =
  ...
  | IDENT name -> advance c; Ast.Var name
  | LPAREN ->
      advance c;
      let e = p_expr c in
      expect c RPAREN "expected ')'";
      e
  | MINUS -> advance c; Ast.Neg (p_factor c)
...
and p_expr (c : cursor) : Ast.expr =
  ...
  while !continue do
    match peek c with
    | PLUS -> advance c; left := Ast.Add (!left, p_term c)
    | MINUS -> advance c; left := Ast.Sub (!left, p_term c)
    | _ -> continue := false
  done;
  !left
