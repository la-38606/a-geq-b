...
let string_of_monomial (vars : vars) (m : Monomial.t) : string =
  List.mapi
    (fun i e ->
      if e = 0 then None
      else if e = 1 then Some (name_of vars i)
...
let string_of_poly (vars : vars) (p : Polynomial.t) : string =
  if Polynomial.is_zero p then "0"
  else begin
    ...
    List.iteri
      (fun idx (m, c) ->
        ...
        (if idx = 0 then (if neg then Buffer.add_string buf "-")
        ...
        if mono = "" then Buffer.add_string buf mag (* constant term *)
        else if Rational.equal (Rational.abs c) Rational.one then
          Buffer.add_string buf mono (* coefficient +/-1 is implicit *)
        else (Buffer.add_string buf mag; Buffer.add_char buf '*'; Buffer.add_string buf mono))
      terms;
    Buffer.contents buf
  end
  ...
  if Polynomial.is_zero p then "0"
    ...
    List.iteri
      (fun idx (m, c) ->
        ...
        if mono = "" then Buffer.add_string buf mag
        else if Rational.equal (Rational.abs c) Rational.one then
          Buffer.add_string buf mono
        else (Buffer.add_string buf mag; Buffer.add_string buf mono))
      terms;
    Buffer.contents buf
  end
