...
let add_coeff (m : Monomial.t) (c : Rational.t) (p : t) : t =
  M.update m
    (fun cur ->
      let s = match cur with None -> c | Some x -> Rational.add x c in
...
let normalize (p : t) : t =
  M.fold
    (fun m c acc ->
      if Rational.is_zero c then acc
      else add_coeff (Monomial.canonical m) c acc)
    p zero
