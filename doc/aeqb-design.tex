...
\begin{abstract}
\noindent
A\,$\ge$\,B is a standalone system that proves polynomial inequalities of the
form $A \ge B$ by rewriting them as $p = A - B \ge 0$ and certifying $p \ge 0$
with a \emph{sum-of-squares} (SOS) decomposition $p = \sum_i c_i\,q_i^2$ with
$c_i \in \Q_{\ge 0}$ and $q_i \in \Q[x_1,\dots,x_n]$. The system is built in the
LCF tradition: an \emph{untrusted} search engine proposes certificates, and a
small, \emph{trusted}, exact-rational \emph{checker} is the sole authority on
whether an inequality is proved. This document develops the mathematics from
first principles---nonnegativity, sum-of-squares certificates and their
soundness, the Gram-matrix / semidefinite reformulation, \emph{Newton-polytope
basis reduction}, and exact rational extraction via $LDL^{\top}$---and then
describes the concrete system: exact
representations, the trusted checker, the prover, denominator clearing, the
validation harness, and the road to a formally verified checker in Lean~4.
\end{abstract}
...
Deciding whether a multivariate polynomial is nonnegative on $\R^n$ is, in
general, NP-hard. A practical and widely used relaxation restricts attention to
polynomials that admit a \emph{certificate} of nonnegativity in the form of a
sum of squares. Such certificates are attractive for two reasons. First, they
are \emph{sound by inspection}: a sum of squares (with nonnegative coefficients)
is manifestly nonnegative. Second, searching for them reduces to
\emph{semidefinite programming}, a tractable convex problem.
...
The organising principle is the separation, due to the LCF family of proof
assistants, between an \emph{untrusted engine} and a \emph{trusted kernel}. The
engine may use any heuristic, numerical routine, or approximate solver; whatever
it produces is re-verified, exactly, by a small checker. The program never
reports \textsc{Proved} unless the checker accepts. Consequently a bug in the
(complicated) search can only cause a \emph{missed} proof, never a false one.
At the heart of the search is a \emph{Newton-polytope reduction} that fixes the
monomial basis of the sum-of-squares problem canonically and minimally
(Section~\ref{sec:newton}).
Section~\ref{sec:foundations} develops the mathematics; Sections~%
\ref{sec:representation}--\ref{sec:clearing} describe the system; Section~%
\ref{sec:design} gives the architecture; and Section~\ref{sec:roadmap} sketches
the remaining work, including formal verification of the checker in Lean~4.
...
The construction proceeds by evaluating each subexpression to a pair
$(\text{numerator}, \text{denominator})$ and combining fractions; when the input
...
search) and \texttt{check <cert.json>} (load and verify a certificate),
reporting one of four statuses with matching exit codes:
\textsc{Proved}, \textsc{No\_Cert\_Found}, \textsc{Invalid\_Input},
\textsc{Check\_Failed}.
...
Correctness is defended empirically by a benchmark corpus of $119$ inequalities,
each tagged by category and by the status the system should return
(Table~\ref{tab:corpus}). The corpus is drawn from classical results (AM--GM,
Cauchy--Schwarz, Schur), named competition problems (IMO, APMO, and national
olympiads), and a standard olympiad reference; it also includes deliberate
soundness traps.
...
Four extensions complete the design; each preserves the checker as the sole
authority.

\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{Newton-polytope basis and a wide semidefinite prover.} Build the
    monomial basis by the Newton-polytope reduction \eqref{eq:newton-basis} and
    replace the bounded grid search by a genuine SDP over the affine family
    \eqref{eq:match}: the exact core emits the Gram program, a numerical solver
    (e.g.\ an interior-point method) returns an approximate PSD $Q$, and the core
    recovers an exact certificate by \emph{Peyrl--Parrilo rounding}---projecting
    the numerical Gram onto the affine subspace \eqref{eq:match}, which is
    rational and remains PSD when the numerical solution is strictly
    interior---before re-verifying. The numerical code is thus cleanly
    quarantined behind the trust boundary.
  \item \textbf{Constrained inequalities.} Admit side constraints
    ($a \ge 0$, $abc = 1$, triangle conditions) and search for
    \emph{Positivstellensatz} certificates $p = \sigma_0 + \sum_i \sigma_i\,g_i$
    with each $\sigma_i$ a sum of squares and $g_i$ a constraint. This subsumes
    the bulk of olympiad inequalities.
  \item \textbf{A formally verified checker in Lean~4.} Reimplement the checker
    over \texttt{MvPolynomial (Fin n) \(\Q\)} and prove its soundness theorem,
    \[
      \texttt{check\_sos}\ p\ \mathit{cert} = \texttt{true}
      \;\Longrightarrow\; \forall\, \x : \mathrm{Fin}\,n \to \R,\ 0 \le p.\mathrm{eval}\,\x,
    \]
    which is exactly Proposition~\ref{prop:checker} discharged mechanically. The
    proof follows the pattern of Theorem~\ref{thm:soundness}
    (\texttt{sq\_nonneg}, \texttt{mul\_nonneg}, \texttt{Finset.sum\_nonneg}) once
    \texttt{check\_sos} establishes the identity. This upgrades the trusted core
    from ``small and audited'' to machine-verified. (Lean~4 compiles to native
    code, so the verified checker also runs.)
  \item \textbf{Interface.} Because the engine is pure functional code, it can
    be compiled to run in a browser, exposing a small structured interface that
    renders the existing \LaTeX{} proof output.
\end{enumerate}

Together, items~1 and~3 realise a principled three-language architecture: exact
symbolic computation and the trusted core in a typed functional language,
numerical semidefinite programming (untrusted) in the scientific-computing
ecosystem, and a machine-checked proof of the kernel in a proof assistant.
