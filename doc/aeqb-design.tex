...
soundness, the Gram-matrix / semidefinite reformulation, and exact rational
extraction via $LDL^{\top}$---and then describes the concrete system: exact
...
\emph{semidefinite feasibility problem}. Two regimes arise, treated next.
...
For $\deg p > 2$ one enlarges the basis $\z$ (for a homogeneous target of degree
$2d$, the monomials of degree $d$; more economically, the ``square roots'' of the
perfect-square monomials occurring in $p$). Two situations occur.
...
\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{Wide semidefinite prover.} Replace the bounded grid search by a
    genuine SDP over the affine family \eqref{eq:match}: the exact core emits the
    Gram program, a numerical solver (e.g.\ an interior-point method) returns an
    approximate PSD $Q$, and the core rounds it to an exact rational PSD matrix
    and re-verifies. This is the natural home for numerical code, cleanly
